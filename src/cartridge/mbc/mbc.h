...
class MBC {
    protected:
        MBC() = default; // cons only available in subclasses

    public:
        virtual ~MBC() = default;            // decons must be impl for each subclasses
...
class MBC1 final : public MBC {
    public:
        explicit MBC1(const std::vector<uint8_t>& rom, std::vector<uint8_t>& ram) : rom_(ram), ram_(ram) {}
